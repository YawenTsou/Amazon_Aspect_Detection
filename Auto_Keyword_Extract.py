...
class Auto_Keyword_Extract():
    ...
    def __init__(self):
        ...
        self.dictionary_specific = self._seed()
    ...
    def _specific_term(self, threshold):
        result = []
        for i in self.valid_IAC:   
            if i in self.model.wv.vocab:
                number = self._Similarity_count(i, 0.6)
    ...
    def _Similarity_count(self, term, threshold):
        number = {}
        for i in self.category:
            tmp = []
    ...
    def AutoLabeling(self, input_path):
        ...
        threshold = 4
        result = self._specific_term(threshold)
        self.dictionary = self._dic(result)
