...
class Train_Word2vec():
    ...
    def load(self):
        ...
        for i in tqdm(self.sentences):
            self.sentence_list.append(self._review_to_wordlist(i[0].lower()))
        self.nlp.close()
    
    def _review_to_wordlist(self, review):
        # Remove non-letters
...
#             words = [w for w in words if not w in self.stops]
    ...
    def add_data(self, input_path):
        ...
        for i in range(len(dict_list)):
            for j in self._review_to_sentences(dict_list[i]['Text']):
                sentences.append(j)
        sentences = [x for x in sentences if len(x) >= 5]
        for i in tqdm(sentences):
            self.sentence_list.append(self._review_to_wordlist(i.lower()))
        self.nlp.close()
