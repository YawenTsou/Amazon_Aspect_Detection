...
class Get_IAC():
    ...
    def _review_to_sentences(self, review):
    ...
    def InputData(self, input_path):
        ...
        for i in range(len(self.dict_list)):
            for j in self._review_to_sentences(self.dict_list[i]['Text']):
    ...
    def GetIAC(self):
        ...
        for i in tqdm(self.sentences):
            ...
            token = []
            for j in pos:
                wordnet_pos = self._get_wordnet_pos(j[1])
            ...
            if [x for x in dependency if 'nsubj' in x] != []:
                for j in self._Subject_Noun_Rule(parse, dependency, token, pos):
                    self.IAC.append(j)
            else: # Non subject noun relation
                for j in self._Non_Subject_Noun_Rule(parse, dependency, token, pos):
                    self.IAC.append(j)
        self.nlp.close()
        self.IAC = list(set(self.IAC))
        print('Get IAC success.')
    ...
    def _Conj(self, index, dependency, token):
        IAC = []
    ...
    def _Subject_Noun_Rule(self, parse, dependency, token, pos):
        ...
        if adv_mod != [] or adj_mod != []:
            A, B = self._Rule1(adv_mod, adj_mod, active_token, token)
            result += A
            index += B

        #  does not have auxiliary verb   
        if any(k in token for k in be) == False and [x for x in pos if 'MD' in x] == []:
            A, B = self._Rule2(token, pos, dependency, active_token, adv_mod, adj_mod)
            result += A
            index += B

            if [x for x in dependency if 'dobj' in x] != []:
                A, B = self._Rule3(dependency, token, pos)
                result += A
                index += B

            if [x for x in dependency if 'xcomp' in x] != []:
                A, B = self._Rule4(dependency, token, pos)
                result += A
                index += B  

        if [x for x in dependency if 'cop' in x] != []:
            A, B = self._Rule5(dependency, pos, active_token, token)
            result += A
            index += B

        result += self._Conj(index, dependency, token)
        return list(set(result))
    
    # 3.3.3 Rule 1 
    def _Rule1(self, adv_mod, adj_mod, active_token, token):
        IAC = []
        index = []
        if adv_mod != []:
            for j in adv_mod:
    ...
    # 3.3.3 Rule 2-1
    def _Rule2(self, token, pos, dependency, active_token, adv_mod, adj_mod):
        IAC = []
        index = []
    ...
    # 3.3.3 Rule 2-2 & 2-3
    def _Rule3(self, dependency, token, pos):
        IAC = []
        index = []
    ...
    # 3.3.3 Rule 2-4
    def _Rule4(self, dependency, token, pos):
        IAC = []
        index = []
    ...
    def _Rule5(self, dependency, pos, active_token, token):
        IAC = []
        index = []
    ...
    def _Non_Subject_Noun_Rule(self, parse, dependency, token, pos):
        result = []
        index = []
        if [x for x in dependency if 'xcomp' in x] != []:
            A, B = self._Rule6(dependency, token)
            result += A
            index += B

        if [x for x in dependency if 'case' in x] != []:
            A, B = self._Rule7(dependency, pos, token)
            result += A
            index += B

        if [x for x in dependency if 'dobj' in x] != []:
            A, B = self._Rule8(dependency, token)
            result += A
            index += B
    ...
    # 3.3.4 Rule 1
    def _Rule6(self, dependency, token):
        IAC = []
        index = []
        xcomp = [x for x in dependency if 'xcomp' in x] #  open clausal complement
        for j in xcomp:
    ...
    # 3.3.4 Rule 2
    def _Rule7(self, dependency, pos, token):
        IAC = []
        index = []
    ...
    # 3.3.4 Rule 3
    def _Rule8(self, dependency, token):
        IAC = []
        index = []
